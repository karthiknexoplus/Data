...
@login_required
def tcea_members():
    """Display TCEA members page"""
    try:
        # Load TCEA members data
        tcea_data = load_complete_tcea_data()
        return render_template('tcea_members.html', tcea_data=tcea_data, username=session.get('username'))
    except Exception as e:
        flash(f'Error loading TCEA data: {str(e)}', 'error')
        return render_template('tcea_members.html', tcea_data=[], username=session.get('username'))
