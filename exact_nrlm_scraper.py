...
class ExactNRLMScraper:
    def __init__(self):
        ...
        self.current_grampanchayat = None
    ...
    def submit_form_exact(self, encd_value, state_code=None, district_code=None, block_code=None, grampanchayat_code=None):
        """Submit form using the exact same method as the working request"""
        try:
            # Build the URL with parameters
            url = f"{self.base_url}/BlockWiseSHGMemebrsAction.do?methodName=showShgMembers&encd={encd_value}&{self.token_parameter}={self.token_value}"
            ...
            # Add dropdown values if provided
            if state_code:
                form_data['stateCode'] = state_code
                self.current_state = state_code
            if district_code:
                form_data['districtCode'] = district_code
                self.current_district = district_code
            if block_code:
                form_data['blockCode'] = block_code
                self.current_block = block_code
            if grampanchayat_code:
                form_data['grampanchayatCode'] = grampanchayat_code
                self.current_grampanchayat = grampanchayat_code
            
            # Set referer header
            if self.current_grampanchayat:
                referer = f"{self.base_url}/BlockWiseSHGMemebrsAction.do?methodName=showShgMembers&{self.token_parameter}={self.token_value}&encd={self.current_block}"
            elif self.current_block:
                referer = f"{self.base_url}/BlockWiseSHGMemebrsAction.do?methodName=showShgMembers&{self.token_parameter}={self.token_value}&encd={self.current_district}"
            elif self.current_district:
                referer = f"{self.base_url}/BlockWiseSHGMemebrsAction.do?methodName=showShgMembers&{self.token_parameter}={self.token_value}&encd={self.current_state}"
            else:
                referer = f"{self.base_url}/BlockWiseSHGMemebrsAction.do?methodName=showShgMembers&encd=0"
    ...
    def get_districts(self, state_code):
        ...
        try:
            html = self.submit_form_exact(state_code, state_code=state_code)
            if not html:
                return []
    ...
    def get_blocks(self, district_code):
        ...
        try:
            html = self.submit_form_exact(district_code, 
    ...
    def get_grampanchayats(self, block_code):
        ...
        try:
            html = self.submit_form_exact(block_code, 
                                        state_code=self.current_state, 
                                        district_code=self.current_district, 
    ...
    def get_villages(self, grampanchayat_code):
        ...
        try:
            html = self.submit_form_exact(grampanchayat_code, 
                                        state_code=self.current_state, 
                                        district_code=self.current_district, 
                                        block_code=self.current_block, 
    ...
    def get_shg_members(self, village_code=None, grampanchayat_code=None, block_code=None):
        ...
        try:
            ...
            url = f"{self.base_url}/BlockWiseSHGMemebrsAction.do?methodName=showShgMembers&encd={encd}&{self.token_parameter}={self.token_value}"
            ...
            form_data = {
                ...
                self.token_parameter: self.token_value,
                'encd': encd,
                'stateCode': self.current_state,
                'districtCode': self.current_district,
                'blockCode': self.current_block,
                'grampanchayatCode': self.current_grampanchayat,
                'villageCode': village_code or ''
            }
...
def test_exact_scraper():
    scraper = ExactNRLMScraper()
