...
class FinalNRLMScraper:
    ...
    def submit_form(self, encd_value):
        """Submit form with the given encd value"""
        try:
    ...
    def get_districts(self, state_code):
        ...
        try:
            html = self.submit_form(state_code)
            if not html:
                return []
    ...
    def get_blocks(self, district_code):
        ...
        try:
            html = self.submit_form(district_code)
            if not html:
                return []
    ...
    def get_grampanchayats(self, block_code):
        ...
        try:
            html = self.submit_form(block_code)
            if not html:
                return []
    ...
    def get_villages(self, grampanchayat_code):
        ...
        try:
            html = self.submit_form(grampanchayat_code)
            if not html:
                return []
        ...
        if state_01:
            ...
            if districts:
                ...
                if blocks:
                    ...
                    block = blocks[0]
                    print(f"\n=== Testing with {block['name']} ===")
                    
                    # Get grampanchayats
                    grampanchayats = scraper.get_grampanchayats(block['code'])
                    if grampanchayats:
                        # Test with first grampanchayat
                        gp = grampanchayats[0]
                        print(f"\n=== Testing with {gp['name']} ===")
                        
                        # Get villages
                        villages = scraper.get_villages(gp['code'])
                        if villages:
                            # Test with first village
                            village = villages[0]
                            print(f"\n=== Testing with {village['name']} ===")
                            
                            # Get SHG members
                            members = scraper.get_shg_members(village_code=village['code'])
                            if members:
                                print(f"\n=== Found {len(members)} SHG members ===")
                                for i, member in enumerate(members[:3]):  # Show first 3
                                    print(f"  {i+1}. {member['shg_name']} - {member['member_name']} ({member['member_code']})")
                            else:
                                print("No SHG members found")
                        else:
                            print("No villages found")
                    else:
                        print("No grampanchayats found")
                else:
